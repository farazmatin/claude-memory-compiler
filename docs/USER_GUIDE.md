...
Both are safe to re-run and never overwrite an existing `.env`. If either reports
a missing prerequisite, install it and run again.

`setup.ps1` does the whole Windows install: ffmpeg and uv through winget or
Chocolatey, waiting for Docker Desktop, the Python environment, the containers and
their models, the folders and manifest, the nightly Scheduled Task, and a final
verification pass. It stops twice to ask you for something only you can give it —
a HuggingFace token, and Google Drive consent. Both are covered below.

To re-run just the verification later:

```powershell
.\scripts\setup.ps1 -VerifyOnly
```

That checks `doctor`, a Drive capture dry run, the Postgres-backed index, the
diarization token, the dashboard, and whether the nightly task is registered.

The rest of this section is what the setup scripts do, for when you want to do it
by hand or something went wrong.
...
That copies `.env.example` to `.env` and generates the two secrets that can be
generated locally. It never touches a value that is already set, so re-running it
cannot rotate the password your database is still using.

| Variable | How it gets set |
|---|---|
| `MMC_LIGHTRAG_API_KEY` | generated by `config init` |
| `POSTGRES_PASSWORD` | generated by `config init` |
| `HF_TOKEN` | a HuggingFace **read** token from hf.co/settings/tokens — yours to create |

The token goes in without ever appearing on a command line:

```bash
uv run pipeline config set HF_TOKEN     # paste the token, then Ctrl-D
uv run pipeline config show             # configured / missing, never the values
```
...
`pipeline doctor` tells the two apart: it checks that the gated model is actually
reachable with your token, not just that a token exists.
...
`.env` is read by the pipeline itself as well as by `docker compose`, so a value
set there reaches both. A real environment variable always wins over the file, so
a scheduled task can still override one without editing anything shared.
...
`doctor` prints a fix next to anything wrong. Get it to zero failures before
continuing. Warnings are usually fine to start with, with one exception worth
reading: if `nightly task` warns, nothing is scheduled to run the batch, and the
corpus will stop growing the moment you stop typing `pipeline run` yourself.

Nothing in the output is a secret — every configuration check says `configured`
or `missing` — so the report is safe to paste into an issue.

### 2.5 Authorize Google Drive (optional, but it is what makes this unattended)

Recordings reach the pipeline through a private Drive folder. Google requires an
interactive consent screen; a script can open it, but it cannot approve on your
behalf. Once:
...
2. Save the downloaded JSON as
   ...
   (Linux/macOS: `~/.local/MeetingMinutesCompiler/drive-client.json`, or wherever
   `MMC_DRIVE_CREDENTIALS` points)
3. Put the folder id in `.env` as `MMC_DRIVE_FUTURE_FOLDER_ID`
4. Run `uv run pipeline auth-drive` and approve read-only access

The refresh token is stored outside the repository and keeps nightly capture
running after that. `pipeline doctor` verifies it by listing the real folder,
because a revoked token looks exactly like a quiet week with no meetings.

```bash
uv run pipeline capture --dry-run   # what it would download, without downloading
```
...
| No new meetings for days | Drive token revoked, or nothing scheduled | `pipeline doctor` — the `drive auth` and `nightly task` checks |
| A setting in `.env` seems ignored | A real environment variable is overriding it | `pipeline config show`; the environment wins over the file |
...
pipeline config init               create .env, generate the local secrets
pipeline config show               which secrets are configured (never the values)
  --key KEY                          one key; exit code is the answer
pipeline config set KEY            set one key, value read from stdin

pipeline auth-drive                one-time Google Drive consent
pipeline capture --dry-run         what Drive capture would download
...
pipeline reindex                   re-push minutes after the index is replaced
  --queue-only                       requeue without indexing yet
