...
RUN_STATUS_HARNESS = REPO_ROOT / "tests" / "js" / "check_run_status.mjs"
...
def test_run_status_line_reports_a_run_the_dashboard_did_not_start():
    """The masthead used to say "idle" through an entire shell run."""
    result = subprocess.run(
        ["node", str(RUN_STATUS_HARNESS)],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
    )
